...
# Autoplot gives: 1) residuals vs fitted, 2) QQ, 3) scale-location, 4) Cook’s
autoplot(model_ols, which = 1:4, ncol = 2)

# 3. Build Weights & Weighted Least Squares

# absolute residuals ~ fitted to model the variance function
abs_resid <- abs(residuals(model_ols))
var_fit   <- lm(abs_resid ~ fitted(model_ols))
w         <- 1 / (predict(var_fit)^2)          # inverse-variance weights
...
# 6. Additional Diagnostics for Final Choice (assume WLS)

# Residual plots
autoplot(model_wls, which = 1:4, ncol = 2)

# Normality test
shapiro.test(residuals(model_wls))

# Influence statistics
influence_measures(model_wls) |> tidy()


# 7.  Hold-Out Validation (70/30 split)

set.seed(3220)
idx_train <- createDataPartition(full_df$life_expectancy, p = 0.70, list = FALSE)

train_df  <- full_df[idx_train, ]
test_df   <- full_df[-idx_train, ]
...
fit_train <- lm(
  life_expectancy ~ median_household_income +
    ...
    primary_care_physicians_ratio,
  data    = train_df,
  weights = w[idx_train]            # reuse weights for training rows
)

test_df$pred <- predict(fit_train, newdata = test_df)
...
# 9. Cleanup Objects (optional)

rm(abs_resid, var_fit, bc_out)  # drop temp objs

```

We are hoping to apply the Weighted Least Squares' additional technique.
